...
public class Uitbreiding_Gui extends JPanel implements MouseListener {
	...
	public Uitbreiding_Gui(Uitbreiding uitbreiding)
	{
		...
		try {
			nieuweAfbeelding = ImageIO.read(new File(uitbreiding.getAfbeelding()));
		} catch (IOException e) {
		...
		mijnLabel = new JLabel(uitbreiding.getNaam(),mijnIcoontje,JLabel.CENTER);
		mijnLabel.setVerticalTextPosition(JLabel.BOTTOM);
		mijnLabel.setHorizontalTextPosition(JLabel.CENTER);
