...
public class TegelGebiedBeheerder {
	...
	public TegelGebiedBeheerder(Tegel tegel) {
		...
		// 1 extra nodig voor elke hoek ( + 4 dus ) want die wijzen naar beide
		// kanten
		// voor elke kant een aparte lijst bijhouden is het beste

		// voor elk punt in het terrein pointer bijhouden naar aangrenzend punt
		// in de buurtegel
		...
		for (int i = 0; i < aantalKolommen; i++) {
			...
			GebiedHelpersBOVEN.put(new Punt(0, i), new Terrein(null, new Punt(
					aantalRijen - 1, i)));

			// onder is rij aantalRijen - 1, kolom i
			GebiedHelpersONDER.put(new Punt(aantalRijen - 1, i), new Terrein(
					null, new Punt(0, i)));

			// rechts is i, aantalKolommen - 1
			GebiedHelpersRECHTS.put(new Punt(i, aantalKolommen - 1),
					new Terrein(null, new Punt(i, 0)));

			// links is i, 0
			GebiedHelpersLINKS.put(new Punt(i, 0), new Terrein(null, new Punt(
					i, aantalKolommen - 1)));
		}
	}
	...
	public boolean controleerOvereenkomstigeZijden(TegelGebiedBeheerder buur,
			RICHTING richting) {
		// Elk punt in deze zijde heeft een equivalent aan de andere kant op de
		// buur GebiedHelper.
		// Dat equivalent zoeken we op op terreinType om zo de overeenkomsten te
		// bepalen
		...
		for (Punt huidigPunt : punten) {
			...
			if (huidigTerrein == TerreinTypeVerzameling.getInstantie().getType(
					"TerreinType_Wildcard"))
				continue;

			if ((huidigTerrein != buurTerrein)
					&& (buurTerrein != TerreinTypeVerzameling.getInstantie()
	...
	public void setBuur(Tegel buur, RICHTING richting) {
		// moeten zelf niet verder in de boom van tegels zoeken, daar zorgt
		// tegel wel voor
	...
	public Gebied getBeperktGebied(Terrein start) {
		...
		// rondom het startterrein zoeken naar aangrenzende stukjes terrein van
		// hetzelfde type
	 ...
	 */
	// TODO : gesloten (de check op dubbele terreinen) naar gebied brengen, niet
	// in functie houden
	private void berekenGebied(Gebied gebied, Terrein start,
			Vector<String> gesloten, boolean beperkTotTegel) {
		...
		if (tegel.getTerreinType(start.getPositie()) == gebiedType) {
			...
			if (!gesloten.contains(nieuw.toString())) {
				...
				for (Tegel.RICHTING richting : Tegel.RICHTING.values()) {
					...
					// slechts de starttegel bekijken, niet verdergaan naar
					// aangrenzende
					if (beperkTotTegel)
						doorgaanNaarVolgende = false;
					...
					if (doorgaanNaarVolgende) {
						// correctie van vorige increment (die zit dan al te
						// ver);
						punt.setX(punt.getX() - xincrement);
						punt.setY(punt.getY() - yincrement);

						nieuw = tegel.getGebiedBeheerder().getGebiedHelpers(
								richting).get(punt);

						// nieuw bestaat normaal gezien altijd, gezien de
