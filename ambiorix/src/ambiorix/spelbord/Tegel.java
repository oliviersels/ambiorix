...
public class Tegel implements TegelBasis {
	...
	/*
	 * Roteert de tegel met een bepaald aantal graden. OPGELET : is NIET
	 * incrementeel ! maw : setRotatie(90) en daarna setRotatie(180) betekent
	 * een rotatie van 180, niet van 270 !!!
	 */
	...
	private void berekenGebiedBeginPunten() {
		...
		// Als ze nog niet tot een gevonden gebied behoren, zijn ze het begin
		// van een nieuw gebied
		for (int i = 0; i < terrein.length; i++) {
			for (int j = 0; j < terrein[0].length; j++) {
				...
				if (temp.getType() != TerreinTypeVerzameling.getInstantie()
						.getType("TerreinType_Wildcard")) {
					...
					if (isBeginPunt) {
						...
						gevondenGebieden.add(gebiedBeheerder
								.getBeperktGebied(temp));
					}
				}
			}
		}
	...
	public boolean kanBuurAccepteren(Tegel buur, RICHTING richting) {
		// terreintypes in aangrenzende vakjes van de terreinmatrix checken en
		// zien of ze overeenkomen

		// we maken gebruik van TegelGebiedBeheerder
		boolean output = this.gebiedBeheerder.controleerOvereenkomstigeZijden(
				buur.getGebiedBeheerder(), richting);

		// ook omliggende buren nog checken !!!!
		/*
		 * RICHTING[] pad1 = BuurPaden.getPad(richting, 1); RICHTING[]
		 * operaties1 = BuurPaden.getOperaties(richting, 1); RICHTING[] pad2 =
		 * BuurPaden.getPad(richting, 2); RICHTING[] operaties2 =
		 * BuurPaden.getOperaties(richting, 2);
		 * 
		 * output = output && kanBuurAccepterenUitgebreid(buur, pad1,
		 * operaties1); output = output && kanBuurAccepterenUitgebreid(buur,
		 * pad2, operaties2);
		 */
	...
	/*
	 * private boolean kanBuurAccepterenUitgebreid(Tegel buur, RICHTING[] pad,
	 * RICHTING[] operaties) { int teller = 0; Tegel tegel = this; boolean
	 * output = true;
	 * 
	 * while( teller < pad.length ) { tegel = tegel.getBuur( pad[teller] ); if(
	 * tegel == null ) return output;
	 * 
	 * if( operaties[teller] != null ) output =
	 * tegel.getGebiedBeheerder().controleerOvereenkomstigeZijden
	 * (buur.getGebiedBeheerder(), operaties[teller]);
	 * 
	...
	public void setBuur(Tegel buur, RICHTING richting) {
		...
		// moeten ons geen zorgen meer maken over andere buren. Spelbord regelt
		// dit.
	}
	...
	public void plaatsPion(Punt positie, Pion pion) {
		boolean bestaat = (positie.getX() < pionPosities.length)
				&& (positie.getY() < pionPosities[0].length);
	...
	/*
	 * Gaat het eerste terrein teruggeven dat hij op deze tegel vind van het
	 * gespecifieerde type Dit is handig als we een gebied moeten berekenen.
	 */
	...
	// TODO : protected maken ipv public, maar dan beginnen de Tests moeilijk te
	// doen
	...
	public String toXML() {
		...
		for (RICHTING richting : RICHTING.values()) {
			if (buren[richting.ordinal()] != null) {
				// buur met lager ID nemen, anders kunnen we later in de
				// problemen komen bij het
				// terug inladen.
				if (buren[richting.ordinal()].getID() < this.ID) {
					output += "\t\t\t<buur>\n";
					output += "\t\t\t\t<id>"
							+ this.buren[richting.ordinal()].getID()
							+ "</id>\n";
					output += "\t\t\t\t<richting>" + richting.toString()
							+ "</richting>\n";
					output += "\t\t\t</buur>\n";
					break;
				}
	...
	/*
	 * Deze functie maakt de tegel aan met de gegeven type, id en rotatie. De
	 * tegel op het spelbord plaatsen (buren toekennen) en de pionnen
	 * eropzetten, moet gedaan worden via spelbord, niet in deze functie.
	 */
	public static Tegel fromXML(XmlNode input) {
		...
		Tegel output = new Tegel(TegelTypeVerzameling.getInstantie().getType(
				type));
		output.setID(id);
