...
public class Pion implements PionBasis {
	...
	public String toXML(Punt locatie) {
		...
		/*
		 * if(speler != null) output += "\t\t\t\t\t<speler>" +
		 * this.speler.getNaam() + "</speler>\n"; else output +=
		 * "\t\t\t\t\t<speler>" + "1" + "</speler>\n";
		 */
	...
	public static Pion fromXML(XmlNode input) {
		// TODO : speler zien vast te krijgen !!! of mss speler gewoon van pion
		// weglaten ? interesting...
		...
		return new Pion(id, PionTypeVerzameling.getInstantie().getType(type),
				null); // FIXME: Oli fixt maar robin doet de rest!
	}
}
