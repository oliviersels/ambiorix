...
public class Spelbord {
	...
	// som van alle tegels die nog over zijn. Als deze op 0 staat is de pool
	// leeg, en is het spel gedaan
	...
	public Spelbord() {
		/*
		 * this.beginTegel = beginTegel; this.beginTegel.setID(0);
		 * 
	...
	/*
	 * Geeft een pointer naar de volgende tegel, numerieke ID en TegelType juist
	 * ingesteld. Geeft null als er geen tegels meer over zijn (spel is gedaan)
	 */
	...
	private TegelType getRandomTegelType() {
		...
		overgeblevenTegels.put(gevondenType, overgeblevenTegels
				.get(gevondenType) - 1);
	...
	/*
	 * Deze functie controleert of tegel plaatsbaar is naast buur, in de
	 * opgegeven richting. Ze gaat echter nog verder en controleert ook meteen
	 * de andere mogelijke buren van de nieuwe tegel of hij daar wel aan kan
	 * grenzen.
	 * 
	...
	public boolean controleerPlaatsbaarheid(Tegel tegel, BordPositie positie) {
		...
		Punt nieuweCoordinaat = getAangrenzendeCoordinaat(buurCoordinaat,
				richting);
	...
	/*
	 * Deze functie controleer of een pion plaatsbaar is op terrein door het
	 * gebied rondom terrein te berekenen en te zien of er nog geen andere
	 * pionnen aanwezig zijn.
	 */
	public boolean controleerPlaatsbaarheid(Pion pion, Terrein terrein) {
		...
		// als het terrein niet toegankelijk is, kunnen we zoiezo geen pion erop
		// plaatsen
	...
	/**
	 * Deze functie gaat controleren of een tegel ERGENS op het bord kan
	 * geplaatst worden. Als dit zo is, is output.size() != 0
	 * 
	 * Hij gaat alle tegels afgaan, in de 4 richtingen kijken of er al een buur
	 * staat. Is dit niet zo controleert hij de plaatsbaarheid van de tegel.
	 * Geeft dit true terug, wordt de BordPositie toegevoegd aan de output.
	 * 
	 * ALs stopDirect == false zal hij een lijst teruggeven van alle mogelijke
	 * posities waar de tegel kan gelegd worden. = trager => maar handig voor de
	 * AI Als stopDirect == true zal hij enkel de eerste die hij vind teruggeven
	 * en dan stoppen. = snel => om een voorgestelde tegel te checken
	 * 
	 */
	public Vector<BordPositie> controleerGlobalePlaatsbaarheid(Tegel tegel,
		...
		// deze tegels allemaal afgaan en aan elke zijde checken of de nieuwe
		// tegel plaatsbaar is.
	...
	/*
	 * Deze functie verwijdert de tegel van het spelbord, en past ook de buren
	 * aan.
	 */
	public void verwijderTegel(Tegel tegel) {
		...
		// de buren updaten zodat ze geen verwijzingen meer hebben naar de
		// verwijderde tegel
		for (Tegel.RICHTING richting : Tegel.RICHTING.values()) {
			...
			if (buurPositie != null) {
				...
				if (buur != null) {
					...
					// tegel zelf ook aanpassen, mss wille we hem terug opnieuw
	...
	/*
	 * verwijdert een pion van de opgegeven positie, als daar eentje staat
	 * tenminste.
	 * 
	 * OPM : dit verwijdert niet het pion-object, slechts de verwijzing ernaar
	 * op het spelbord.
	 */
	...
	/*
	 * Zoekt waar de pion staat en verwijdert deze dan van het spelbord. Deze
	 * functie is traag => Gebruikt verwijderPion(Terrein positie)!!!
	 * 
	 * OPM : zoekt maar tot hij de pion 1x gevonden heeft. Als meermaals
	 * dezelfde pion op het spelbord staat , zal enkel de eerste verwijzing
	...
	/*
	 * Deze functie gaat tegel toevoegen aan het spelbord, naast de
	 * gespecifieerde buur. De richting geeft aan hoe de nieuwe tegel ligt tov
	 * de buur. richting == BOVEN betekent bijv. dat de nieuwe tegel BOVEN de
	 * buur moet geplaatst worden.
	 * 
	 * De functie zal ervoor zorgen dat de correcte tegels onderling deze tegel
	 * als buur gaan instellen. De functie moet dus maar 1x aangeroepen worden
	 * per nieuwe tegel.
	 * 
	 * De functie gaat niet zelf controleren of de plaatsing wel geldig is.
	 * Gebruik hiervoor Spelbord::controleerPlaatsbaarheid
	 */
	public void plaatsTegel(Tegel tegel, BordPositie positie) {
		...
		// nu we de coordinaat kennen, de buren errond laten weten dat de nieuwe
		// tegel gezet is
		for (Tegel.RICHTING r : Tegel.RICHTING.values()) {
			buurCoordinaat = getAangrenzendeCoordinaat(nieuweCoordinaat, r);
			buur = tegelCoordinaten.get(buurCoordinaat);
	...
	private Punt getTegelCoordinaat(Tegel tegel) {
		...
		for (Punt punt : punten) {
			if ((tegelCoordinaten.get(punt)) == tegel)
				return punt;
		}
	...
	private Punt getAangrenzendeCoordinaat(Punt coordinaat,
	...
	/*
	 * Gaat recursief rondom start zoeken voor terreinstukken van hetzelfde
	 * type.
	 */
	...
	public String toXML() {
		...
		output += "\t<beginTegel>\n" + this.beginTegel.toXML()
				+ "\t</beginTegel>\n";

		// niet echt nodig, meer ter controle
		output += "\t<volgendeTegelID>" + this.volgendeTegelID
				+ "</volgendeTegelID>\n";
		...
		for (String tegelType : tegelTypes) {
			...
			output += "\t\t\t<aantal>" + this.overgeblevenTegels.get(tegelType)
					+ "</aantal>\n";
		...
		// heel belangrijk om te sorteren. Anders gaan we bij het herinlezen de
		// buren niet meer tegoei
		// vinden. Dus ook heel belangrijk dat de id's overeenkomen met volgorde
		// van toevoegen op spelbord !!!
	...
	public static Spelbord fromXML(XmlNode root) {
		...
		int volgendeTegelID = Integer.parseInt(root.getChild("volgendeTegelID")
				.getValue());
		output.setVolgendeTegelID(volgendeTegelID);

		Vector<XmlNode> overgeblevenTegels = root
				.getChild("overgeblevenTegels").getChildren("reeks");
		for (XmlNode reeks : overgeblevenTegels) {
			output.setTegelAantal(reeks.getChild("type").getValue(), Integer
					.parseInt(reeks.getChild("aantal").getValue()));
		}
		...
		for (XmlNode tegel : tegels) {
			...
			if (buur == null)// beginTegel
			{
				...
				RICHTING richting = RICHTING.fromString(buur.getChild(
						"richting").getValue());

				Tegel buurTegel = output.getTegel(buurId);
				// TODO : hier gaat hij nog def out in
				output.plaatsTegel(nieuweTegel, new BordPositie(buurTegel,
						richting));
			}
			...
			if (pionnenTag != null) // anders geen pionnen op deze tegel
			{
				...
				for (XmlNode pion : pionnen) {
					...
					Punt locatie = Punt.fromString(pion.getChild("locatie")
							.getValue());

					output.plaatsPion(nieuwePion, new Terrein(nieuweTegel,
							locatie));
				}
			}
