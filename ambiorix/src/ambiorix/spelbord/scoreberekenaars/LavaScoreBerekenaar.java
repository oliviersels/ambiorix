...
public class LavaScoreBerekenaar extends SimpelScoreBerekenaar 
{
	...
	public int berekenScore(Gebied gebied, Speler speler) 
	{
		...
		// als het spel nog niet gedaan is moet de weg volledig zijn voor punten te kunnen krijgen
		...
		// we gaan de score verdubbelen ALS er een tegel met lava onderdeel is van onze mooie weg
		// zoek dus eerst of er een tegel met lava in het gebied zit :)
		for( Tegel tegel : gebied.getTegels() ) 
		{
			if( tegel.getType() == TegelTypeVerzameling.getInstantie().getType("TegelType_LavaMetWeg") )
				result *= 2;
		}
