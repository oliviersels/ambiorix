...
public class UitbreidingImplementatie extends Type implements
		UitbreidingInterface {
	...
	public void bereidVoor(Vector<String> andereUitbreidingen) {
		...
		try {
			...
			// TODO : ik weet dat dit generieker kan in 1 functie, maar for the
			// time begin...

			// TERREINTYPES
			File terreinTypesMap = new File(uitbreidingPad
					+ "ambiorix/spelbord/terreintypes/");

			String[] terreinTypesNamen = terreinTypesMap.list();
			if (terreinTypesNamen != null) {
				KlasseLader<TerreinType> terreinLader = new KlasseLader<TerreinType>(
						uitbreidingPad);
				...
				for (String terreinTypeNaam : terreinTypesNamen) {
															...
															// geldige file
					{
						TerreinType terreinType = terreinLader.LaadKlasse(
								"ambiorix.spelbord.terreintypes."
										+ getClassNaam(terreinTypeNaam))
								.newInstance();
						TerreinTypeVerzameling.getInstantie().registreerType(
								terreinType);
					}
				}
			}
			...
			// TEGELTYPES
			File tegelTypesMap = new File(uitbreidingPad
					+ "ambiorix/spelbord/tegeltypes/");

			String[] tegelTypesNamen = tegelTypesMap.list();
			if (tegelTypesNamen != null) {
				KlasseLader<TegelType> tegelLader = new KlasseLader<TegelType>(
						uitbreidingPad);
				...
				for (String tegelTypeNaam : tegelTypesNamen) {
															...
															// file
					{
						TegelType tegelType = tegelLader.LaadKlasse(
								"ambiorix.spelbord.tegeltypes."
										+ getClassNaam(tegelTypeNaam))
								.newInstance();
						TegelTypeVerzameling.getInstantie().registreerType(
								tegelType);
					}
				}
			}
			...
			// PIONTYPES
			File pionTypesMap = new File(uitbreidingPad
					+ "ambiorix/spelbord/piontypes/");

			String[] pionTypesNamen = pionTypesMap.list();
			if (pionTypesNamen != null) {
				KlasseLader<PionType> pionLader = new KlasseLader<PionType>(
						uitbreidingPad);
				...
				for (String pionTypeNaam : pionTypesNamen) {
															...
															// file
					{
						PionType pionType = pionLader.LaadKlasse(
								"ambiorix.spelbord.piontypes."
										+ getClassNaam(pionTypeNaam))
								.newInstance();
						PionTypeVerzameling.getInstantie().registreerType(
								pionType);
					}
				}
			}
			...
			File actiesMap = new File(uitbreidingPad
					+ "ambiorix/acties/specifiek/");

			String[] actieNamen = actiesMap.list();
			if (actieNamen != null) {
				KlasseLader<AbstractActie> actieLader = new KlasseLader<AbstractActie>(
						uitbreidingPad);
	...
	public Ai getAi() {
		...
		File aiMap = new File(uitbreidingPad + "ambiorix/ai/");
		...
		try {
			...
			ai = aiLader.LaadKlasse(
					"ambiorix.ai." + getClassNaam(aiNamen[0].getName()))
					.newInstance();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public ScoreBerekenaar getScoreBerekenaar() {
		...
		File berekenaarsMap = new File(uitbreidingPad
				+ "ambiorix/spelbord/scoreberekenaars/");
		...
		try {
			KlasseLader<ScoreBerekenaar> berekenaarLader = new KlasseLader<ScoreBerekenaar>(
					uitbreidingPad);
			berekenaar = berekenaarLader.LaadKlasse(
					"ambiorix.spelbord.scoreberekenaars."
							+ getClassNaam(berekenaarsNamen[0].getName()))
					.newInstance();
		} catch (Exception e) {
			e.printStackTrace();
		}
