...
public class Spel {
	...
	public Spel(Uitvoer gui) {
		...
		scoreBerekenaar = UitbreidingVerzameling.getInstantie()
				.getScoreBerekenaar();
	...
	public void start() {
		...
		try {
			String startString = UitbreidingVerzameling.getInstantie()
					.getEersteActie();
			Object[] param = { speltoolkit, null };
			Class<?>[] paramKlassen = { SpelToolkit.class, AbstractActie.class };
			Actie startActie = ActieVerzameling.getInstantie().getNewInstantie(
					startString, param, paramKlassen);
