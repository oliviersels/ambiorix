...
public class StandaardAi extends Ai {
	...
	public Vector<Antwoord> berekenZet() {
		...
		// System.out.println("te plaatsen tegel: " +
		// tegels.elementAt(0).getType().getID());
		...
		for (int i = 0; i < positieLijst.size(); ++i) {
			...
			// worden
			{
				...
				// System.out.println("i+1 VOOR toekenning " +
				// positieLijst.elementAt(i+1).getScore(speler) );
				positieLijst.elementAt(i).setScore(speler, tijdelijkTotaal);
				// System.out.println("i+1 NA toekenning " +
				// positieLijst.elementAt(i+1).getScore(speler) );
			}
	...
	public Vector<Positie> maakPosities(Spelbord bord) {
		...
		for (int i = 0; i < BPLijst.size(); ++i) {
			...
			if (tegels.elementAt(0).getType().getID().equals(
					"TegelType_LavaMetBurchten")
					|| tegels.elementAt(0).getType().getID().equals(
							"TegelType_BBBBB")) {
		...
		for (int i = 0; i < antwoorden.size(); ++i) {
			if (i == 0) {
				...
				for (int j = 0; j < antwoorden.elementAt(i).getTegels().size(); ++j) {
					System.out.println(antwoorden.elementAt(i).getTegels()
				...
				for (int j = 0; j < antwoorden.elementAt(i).getTerreinen()
						.size(); ++j) {
					System.out.println(antwoorden.elementAt(i).getTerreinen()
							.elementAt(j).toString());
				}
			}
		}
	...
	public void telScores(Vector<Positie> lijst) {
		for (int j = 0; j < lijst.size(); ++j) {
			System.out.println("element " + (j + 1) + ": "
					+ lijst.elementAt(j).getScores().size());
		}
	}
	...
	public void printScores(Vector<Positie> lijst) {
		for (int i = 0; i < lijst.size(); ++i) {
			...
			for (int j = 0; j < lijst.elementAt(i).getScores().size(); ++j) {
				System.out.println(" - "
						+ lijst.elementAt(i).getScores().elementAt(j)
								.getScore());
			}
			// System.out.println( "---- " );
		}
